...
function dixie_get_detail(){
    ...
    if($type=='post'||$type=='article'){
      ...
      $tdate = generate_training_date(get_post_detail('training_date',false));
      $udate = generate_training_date(get_post_detail('training_date',false),true);
      echo '<div class="detail">';
      echo '<div class="detail-training-date">Event on '.date('F jS, Y',$tdate).($udate!==$tdate?' - '.date('F jS, Y',$udate):'').'</div>';
