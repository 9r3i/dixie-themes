...
<div class="content">   
      <div id="gallery_origin"><?php get_post_detail('content'); ?></div>

      <div id="gallery_files"></div>

  <div class="clear-both"></div>
</div>

<script type="text/javascript">
var gf = document.getElementById('gallery_files');
/* var imgs = document.getElementsByTagName('img'); */
var imgs = document.querySelectorAll('#gallery_origin img');
for(var i=0;i<imgs.length;i++){
  var image = imgs[i].getAttribute('src');
  var style = imgs[i].getAttribute('style');
  var div = document.createElement('div');
  div.setAttribute('class','each-picture');
  ...
  rel.appendChild(img);
  div.appendChild(rel);
  /* imgs[i].parentElement.insertBefore(div,imgs[i]); */
  /* insertAfter(div,imgs[i]); */
  gf.appendChild(div);
}
var origin = document.getElementById('gallery_origin');
origin.parentElement.removeChild(origin);

/* insert after */
function insertAfter(baru,ref){
  ref.parentNode.insertBefore(baru,ref.nextSibling);
}
</script>
